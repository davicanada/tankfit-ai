...
function statusPresentation(status: string) {
  if (status === "compatible") {
    return { label: "Compatible", className: "bg-emerald-400/15 text-emerald-300", icon: CheckCircle2 };
  }
  if (status === "out_of_scope") {
    return { label: "Out of scope", className: "bg-red-400/15 text-red-300", icon: XCircle };
  }
  return { label: "Technical review required", className: "bg-amber-400/15 text-amber-200", icon: CircleAlert };
}
...
export function CompatibilityAdvisor({ products, presets, initialPresetId }: CompatibilityAdvisorProps) {
  ...
  const [requirements, setRequirements] = useState<CompatibilityRequirements>({ ...initialPreset.requirements });

  const result = useMemo(
    () => evaluateCompatibility(products, requirements),
  ...
  function updateRequirement<Key extends keyof CompatibilityRequirements>(
    key: Key,
    value: CompatibilityRequirements[Key],
  ) {
        ...
        <Card>
          <CardHeader>
            ...
            <p className="text-sm leading-6 text-muted-foreground">
              Editing any value creates a custom scenario and immediately reruns the same deterministic rules.
            </p>
          </CardHeader>
          <CardContent className="grid gap-5 sm:grid-cols-2">
            <RequirementSelect
              ...
              onValueChange={(value) => updateRequirement("material", value as CompatibilityRequirements["material"])}
            />
            <RequirementSelect
              label="Tank type"
              value={requirements.tankType}
              options={selectOptions.tankType}
              onValueChange={(value) => updateRequirement("tankType", value)}
            />
            <RequirementSelect
              label="Existing instrumentation"
              value={requirements.existingInstrumentation}
              options={selectOptions.existingInstrumentation}
              onValueChange={(value) => updateRequirement("existingInstrumentation", value)}
            />
            <RequirementSelect
              label="Gauge interface"
              value={requirements.gaugeInterface}
              options={selectOptions.gaugeInterface}
              onValueChange={(value) => updateRequirement("gaugeInterface", value as CompatibilityRequirements["gaugeInterface"])}
            />
            <RequirementSelect
              label="Connectivity"
              value={requirements.connectivity}
              options={selectOptions.connectivity}
              onValueChange={(value) => updateRequirement("connectivity", value as CompatibilityRequirements["connectivity"])}
            />
            <RequirementSelect
              label="Site distribution"
              value={requirements.siteDistribution}
              options={selectOptions.siteDistribution}
              onValueChange={(value) => updateRequirement("siteDistribution", value as CompatibilityRequirements["siteDistribution"])}
            />
            <RequirementSelect
              label="Measurement preference"
              value={requirements.measurementPreference}
              options={selectOptions.measurementPreference}
              onValueChange={(value) => updateRequirement("measurementPreference", value)}
            />
            <RequirementSelect
              label="Regulated location"
              value={String(requirements.regulatedLocation)}
          ...
          {result.primaryRecommendation ? (
            <div>
              <div className="grid gap-5 sm:grid-cols-[150px_1fr] sm:items-center">
                <div className="relative aspect-square overflow-hidden rounded-xl bg-muted/50">
                  <Image
                    src={result.primaryRecommendation.product.image.path}
                    alt={result.primaryRecommendation.product.image.alt}
                    fill
                    sizes="150px"
              ...
              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <dl className="mt-3 space-y-3 text-xs">
                    <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Rule version</dt><dd className="font-mono">{result.ruleVersion}</dd></div>
                    <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Catalog version</dt><dd className="font-mono">2026.08.1</dd></div>
                    <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Match score</dt><dd className="font-mono">{result.primaryRecommendation.score}</dd></div>
                    <div className="flex justify-between gap-4"><dt className="text-muted-foreground">Preset privilege</dt><dd className="font-mono">None</dd></div>
                  </dl>
                </div>
              </div>
            ...
            <div className="py-12 text-center">
              ...
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted-foreground">
                The engine will not invent a product. Change the confirmed inputs or route this fictional application to technical review.
              </p>
            </div>
          )}
          ...
          {result.reasons.length > 0 ? (
            <div className="mt-7 rounded-lg border border-border bg-background/60 p-4">
              ...
              <ul className="mt-2 space-y-1 text-sm">
                {result.reasons.map((reason) => <li key={reason}>• {humanizeCatalogValue(reason)}</li>)}
              </ul>
            </div>
          ) : null}
        </CardContent>
  ...
  return (
    <div>
      ...
      <Select value={value} onValueChange={onValueChange}>
        <SelectTrigger id={id} className="mt-2 w-full"><SelectValue /></SelectTrigger>
        <SelectContent>
          {options.map((option) => (
            <SelectItem key={option} value={option}>{humanizeCatalogValue(option)}</SelectItem>
          ))}
        </SelectContent>
      </Select>
    </div>
